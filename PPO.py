...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, action_std_init, device = "cpu"):
...
class PPO(Model):
    def __init__(self, 
        ...
        self.policy = ActorCritic(state_dim, action_dim, action_std_init, device)
        self.optimizer = torch.optim.Adam([
            {'params': self.policy.actor.parameters(), 'lr': actor_lr},
            {'params': self.policy.critic.parameters(), 'lr': critic_lr}
        ])

        self.policy_old = ActorCritic(state_dim, action_dim, action_std_init, device)
    ...
    def decay_action_std(self, action_std_decay_rate, min_action_std):
        print("----------------------Decaying action std------------------------")
        self.action_std = self.action_std - action_std_decay_rate
        self.action_std = round(self.action_std, 4)
        if(self.action_std <= min_action_std):
            self.action_std = min_action_std
            print("Action std decayed to min value :", self.action_std)
        else:
            print("Action std decayed to :", self.action_std)
        print("---------------------------------------------------------------")
    
    def select_action(self, state):
        with torch.no_grad():
            state = torch.tensor(state, dtype=torch.float)
            action, action_logprobs, state_val = self.policy_old.act(state)
    ...
    def optimize_model(self):
        rewards = []
        discounted_reward = 0
        for reward, is_terminal in zip(reversed(self.buffer.reward), reversed(self.buffer.is_terminals)):
            if is_terminal:
                discounted_reward = 0
        ...
        old_states = torch.squeeze(torch.tensor(self.buffer.state)).detach().to(self.device)
        old_actions = torch.squeeze(torch.tensor(self.buffer.action)).detach().to(self.device)
        old_logprobs = torch.squeeze(torch.tensor(self.buffer.logprobs)).detach().to(self.device)
        old_state_values = torch.squeeze(torch.tensor(self.buffer.state_values)).detach().to(self.device)
